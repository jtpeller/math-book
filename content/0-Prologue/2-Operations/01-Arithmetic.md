+++
title = "Arithmetic"
description = "Introduces the Arithmetic Operations"
+++
...
Operations are performed on *terms*. {{< dfn "Terms" >}} are most commonly numbers, but as you'll see in algebra, may be variable quantities (like $x$ or $y$).

Operations are most commonly shown as a term, then an operation, then another term, such as:

$$2 + 2$$

This mathematical phrase is known as an {{< dfn "expression" >}}, which is most simply a couple of terms with an operation performed on them.

Now, you may show equality between two expressions via an {{< dfn "equal sign" >}}. An equal sign (that is, $=$) denotes that the two expressions represent the same exact quantity. So, when you see something like: $2 + 2 = 4$, this states that the expression $2+2$ is equal to $4$. This mathematical sentence is known as an {{< dfn "equation" >}}.
...
The Arithmetic Operations are [addition](./2-Addition.md), [subtraction](./3-Subtraction.md), [multiplication](./4-Multiplication.md), and [division](./5-Division.md). These operations are the basis of a large portion of mathematics, and will be explored in the sections below.
...
- {{< dfn "Operand" >}}: Number or *term* that is undergoing the action from an *operator* to produce a solution.
- {{< dfn "Operator" >}}: A symbol used to denote a specific action to be done to the operands.
  - Examples: addition (Symbol: $+$), subtraction (Symbol: $-$), etc.
- {{< dfn "Operation" >}}: Describes the overall actions being done to operands.
