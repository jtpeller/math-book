...
description = "Describes the special characteristics of functions."
+++
...
The {{< dfn "x-intercept" >}} is the point at which the function intersects with the $x$ axis. That is, what value of $x$ results in a value of the function of $0$?

You can find the $x$-intercept by solving for $x$ when $f(x) = 0$.

The other intercept is the {{< dfn "y-intercept" >}} , which is the point at which the function intersects with the $y$ axis. That is, what value of $y$ results from an input value of $x=0$?
...
Functions lacking abrupt changes in value are considered {{< dfn "continuous" >}}.
...
{{< dfn "Discontinuous" >}} functions feature a characteristic known as a {{< dfn "discontinuity" >}}, which is any abrupt change in value.

#### Removable Discontinuity

The "simplest" kind of discontinuity is a {{< dfn "Removable Discontinuity" >}}, which is one in which the function approaches the same values regardless of the direction $x$ is approaching.

{{< image src="image/removable-discontinuity.png" alt="Figure 1.3.1: Removable Discontinuity at $x_0$" >}}
...
>
> A ***hollow circle*** denotes that the value does NOT exist at this location. It indicates a {{< dfn "hole" >}} in the graph.
>
...
Another type of discontinuity is a {{< dfn "Jump Discontinuity" >}}, where the function approaches different values based on the direction $x$ is approaching.

{{< image src="image/jump-discontinuity.png" alt="Figure 1.3.2: Jump Discontinuity at $x_0$" >}}
...
The final kind of discontinuity to discuss is an {{< dfn "Infinite Discontinuity" >}}. This is a special kind of jump discontinuity such that the limits from either side approach infinity (may be both positive, both negative, or one each):

{{< image src="image/infinite-discontinuity.png" alt="Figure 1.3.3: Infinite Discontinuity at $x_0$, for $f(x)=1/x$" >}}

These kinds of discontinuities are also known as a {{< dfn "Vertical Asymptote" >}}.

### Bounded Functions

A {{< dfn "bounded" >}} function is one that is finite.
...
{{< dfn "Unbounded" >}} functions are those that extend toward infinity. Many functions are *unbounded*. For instance, Linear functions are unbounded.
...
- {{< dfn "Bounded above" >}}, which means that only $f(x) \leq k$ holds true. In other words, all function values are less than (or equal to) some real number $k$.
- {{< dfn "Bounded below" >}}, which means that only $f(x) \geq m$ holds true. In other words, all function values are greater than (or equal to) some real number $m$.

## Describing a Function's Shape

### Smooth Functions

A {{< dfn "smooth" >}} function describes one such that the function is *differentiable* everywhere. Put more simply, a smooth function is essentially a special type of continuous function where there are no immediate shifts in its rate of change. For instance, a linear graph is smooth, while the following graph is not smooth. It is a graph of the absolute value of $x$ (that is, $f(x) = |x|$)

{{< image src="image/absolute-graph.png" alt="Figure 1.3.4: Graph of an Absolute Value function" >}}

### Symmetry

A {{< dfn "symmetrical" >}} function describes a function such that the function is the same across an axis.
