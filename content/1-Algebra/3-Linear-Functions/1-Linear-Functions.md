...
description = "Introduces Linear Functions"
+++
...
{{< image src="image/linear-graph.png" alt="Figure 1.4.1: Linear Function, where $f(x)=-2x+2$" >}}
...
{{< image src="image/constant-graph.png" alt="Figure 1.4.2: Constant Function, where $f(x)=4$">}}

***Identity***: A special type of **linear** function where the output is the same as the input. It is of the form, $f(x) = x$.

{{< image src="image/identity-graph.png" alt="Figure 1.4.3: Identity Function, where $f(x)=x$" >}}
...
1. ***Slope*** ($m$): The number describing the direction and steepness of the function's behavior.
   1. Also known as ***rate of change***.
   2. A function's *steepness* is given by the magnitude of the slope (that is, how big).
   3. A function's *direction* is given by the sign of the slope. That is:
...
2. ***Intercept*** ($b$): The constant number which essentially provides a shift of the function's values.
   1. This may also be thought of as the "default" value for $y$, which is the value of $y$ when $x=0$. This is known as the ***$y$-intercept***.
