...
Functions rely on symbols known as $\textcolor{cyan}{\textnormal{variables}}$, which represent a particular quantity that represent some value. These represent an input or an output. Variables are not to be confused with $\textcolor{cyan}{\textnormal{constants}}$, which are simply concrete numbers that do not vary.
...
Now, a $\textcolor{cyan}{\textnormal{formula}}$ is a particular type of equation that models a special situation or relationship between two quantities. It is typically used to represent ways of computing particular quantities that are important, special, or otherwise calculated the same way.
...
A $\textcolor{cyan}{\textnormal{function}}$ is a particular kind of formula that has specific inputs and outputs. More particularly, it is a special formula where every input in its domain has exactly one output.
...
Now, variables can be combined with other variables, constants, etc. in order to create a $\textcolor{cyan}{\textnormal{term}}$, which is simply one single piece that is separated by addition or subtraction.
...
Now, expressions can be classified by the number of terms it contains. A $\textcolor{cyan}{\textnormal{binomial}}$ contains two terms. Meanwhile, a $\textcolor{cyan}{\textnormal{trinomial}}$ contains three terms. While there is a pattern, typically after 3 terms, it's just called a $\textcolor{cyan}{\textnormal{polynomial}}$.
...
$\textcolor{cyan}{\textnormal{Functions}}$ are mathematical models where an input is used to compute an output. More specifically, functions are equations where there is **exactly one output per one input**.

$\textcolor{cyan}{\textnormal{Function}}$ - A formula with inputs and outputs. The output is computed from the inputs based upon the defined relationship.
...
Functions are utilized to discover the output at a particular input. The task of inputting this particular value is called $\textcolor{cyan}{\textnormal{plugging in}}$, and this is one of many interactions you will have with functions.
...
As a side note, that number $2$ is called a *coefficient*. More formally, a $\textcolor{cyan}{\textnormal{coefficient}}$ is a constant number that multiplies with a variable. For instance, for the expression $3x+9y+5$, $3$ is the coefficient of $x$, $9$ is the coefficient of $y$, and 5 is a *constant*.
...
1. $\textcolor{cyan}{\textnormal{Input variable}}$: Describes variables that are provided to the function to be transformed.
   1. Functions with more than one input variable are called $\textcolor{cyan}{\textnormal{multivariate}}$ functions.
2. $\textcolor{cyan}{\textnormal{Output variable}}$: The value that is produced by the function after all operations have been simplified.
   1. Functions can only have one output variable.

Input variable(s) are sometimes referred to as $\textcolor{cyan}{\textnormal{independent variables}}$ (since they do not depend on other values in the function). Instead, they are provided *independently* to the function, and then utilized to calculate the output.

The output variable may be referred to as a $\textcolor{cyan}{\textnormal{depedent variable}}$ since it depends on the independent variables for its value.
...
The set of all values for a particular input variable is the function's $\textcolor{cyan}{\textnormal{domain}}$ while the set of all values for the output variable is the function's $\textcolor{cyan}{\textnormal{range}}$.
...
$\textcolor{cyan}{\textnormal{Intervals}}$ are a consecutive series of numbers that are unique. Intervals typically represent a *continuous* set of numbers (such as "numbers between 0 and 1"), or a *discontinuous* set of numbers, such as "only the numbers 1, 2, or 3".
...
$\textcolor{cyan}{\textnormal{Interval Notation}}$ is essentially shorthand for these inequality operators. This notation utilizes brackets or parentheses to represent a particular range of numbers.
